...
try {
    dbHandleReadonly = openDatabase(DEFAULT_SQLITE_DB_PATH, {createSchema: false, pragmasProfile: "readonly"});
...
if (appState === EAppState.CLEAN_SLATE && !fs.existsSync(DEFAULT_UTXO_DUMP_FILE)) {
    await dumpUtxo();
