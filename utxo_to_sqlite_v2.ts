...
async function main(): Promise<void> {
  ...
  db.exec("CREATE TABLE utxos(txid TEXT, vout INT, value INT, coinbase INT, height INT, scripthash TEXT)");
//   db.exec("CREATE INDEX idx_utxos_scripthash ON utxos(scripthash)");
  ...
  console.log(
    `UTXO Snapshot for ${networkString} at block hash ${Buffer.from(blockHash).reverse().toString("hex")}, contains ${numUtxos} coins`
  );

  const startTime = Date.now();
  const writeBatch: Array<[string, number, number, number, number, string]> = [];
  let coinsPerHashLeft = 0;
  let prevoutHash = "";
  let maxHeight = 0;

  const insertStmt = db.prepare("INSERT INTO utxos VALUES(?, ?, ?, ?, ?, ?)");
  const insertMany = db.transaction((rows: Array<[string, number, number, number, number, string]>) => {
  ...
  for (let coinIdx = 1; coinIdx <= numUtxos; coinIdx++) {
    if (coinsPerHashLeft === 0) {
      ...
      prevoutHash = Buffer.from(hashBuf).reverse().toString("hex");
    ...
    writeBatch.push([prevoutHash, prevoutIndex, amount, isCoinbase, height, scripthash]);
    if (height > maxHeight) maxHeight = height;
    coinsPerHashLeft -= 1;

    if (args.values.verbose) {
      console.log(`Coin ${coinIdx}/${numUtxos}:`);
      console.log(`    prevout = ${prevoutHash}:${prevoutIndex}`);
      console.log(`    amount = ${amount}, height = ${height}, coinbase = ${isCoinbase}`);
