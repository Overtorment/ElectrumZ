...
function decompressPubkey(compressedPubkey: Buffer): Buffer {
	...
	if (
		...
		throw new Error(
			`Invalid compressed pubkey: ${compressedPubkey.toString("hex")}`,
		);
	}
	...
	if (modPow(y, 2n, P) !== rhs)
		throw new Error(
			`Pubkey is not on curve (${compressedPubkey.toString("hex")})`,
		);
...
function decompressScript(reader: StreamingBinaryReader): Buffer {
	...
	if (size === 0) {
		...
		if (scriptSize > 10000)
			throw new Error(`too long script with size ${scriptSize}`);
...
export async function utxoToSqlite(): Promise<void> {
	if (!existsSync(DEFAULT_UTXO_DUMP_FILE)) {
		console.error(
			`Error: UTXO dump file '${DEFAULT_UTXO_DUMP_FILE}' doesn't exist.`,
		);
		process.exit(1);
	}

	const handle = openDatabase(DEFAULT_SQLITE_DB_PATH, {
		createSchema: true,
		pragmasProfile: "bulkload",
	});
	handle.beginImmediate();

	const reader = new StreamingBinaryReader(DEFAULT_UTXO_DUMP_FILE, {
	...
	if (!magicBytes.equals(UTXO_DUMP_MAGIC)) {
		console.error(
			`Error: provided input file '${DEFAULT_UTXO_DUMP_FILE}' is not an UTXO dump.`,
		);
		process.exit(1);
	}
	...
	const networkString =
		NET_MAGIC_BYTES[networkMagic.toString("hex")] ??
		`unknown network (${networkMagic.toString("hex")})`;
	console.log(
	...
	for (let coinIdx = 1; coinIdx <= numUtxos; coinIdx++) {
		...
		if (process.env.VERBOSE) {
			...
			console.log(
				`    prevout = ${prevoutHashBuf.toString("hex")}:${prevoutIndex}`,
			);
		...
		if (coinIdx % (10 * 1024 * 1024) === 0) {
			...
			const etaMinutes =
				progress > 0 ? (elapsed * (1 - progress)) / progress / 60 : 0;
			console.log(
	...
	reader.close();
	console.log(
		`TOTAL: ${numUtxos} UTXOs written to ${DEFAULT_SQLITE_DB_PATH}, snapshot height is ${maxHeight}.`,
	);
	writeFileSync(LAST_PROCESSED_BLOCK_FILE, maxHeight.toString());

	if (!reader.isAtEnd()) {
		console.log(
			`WARNING: input file ${DEFAULT_UTXO_DUMP_FILE} has not reached EOF yet!`,
		);
		process.exit(1);
	}
